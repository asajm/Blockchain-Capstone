...
contract Ownable {
    ...
    modifier onlyOwner() {
        require(_owner == msg.sender, 'Caller has to be the owner');
        _;
    }

    modifier realAddress(address adr) {
        require(adr != address(0), 'The address is not real!');
        _;
    }
...
contract Pausable is Ownable {
    ...
    modifier whenNotPaused() {
        require(_paused == false, 'the operational is paused');
        _;
    }
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != owner, 'The given address is the owner of the given tokenId');

        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == owner || isApprovedForAll(owner, msg.sender), 'Caller is not the owner nor approved for all.');
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(!_exists(tokenId), 'the given tokenId is already exists');
        require(to != address(0), 'The given in invalid');
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(ownerOf(tokenId) == from, 'the "from" address is not the owner of the given tokenId');
...
contract AhmedToken is ERC721Metadata {
    string private _name = 'Ahmed Token';
    ...
    function mint(address to, uint256 tokenId) public onlyOwner returns(bool) {
        super._mint(to, tokenId);
        super._setTokenURI(tokenId, _baseTokenURI);
        return true;
    }
}
